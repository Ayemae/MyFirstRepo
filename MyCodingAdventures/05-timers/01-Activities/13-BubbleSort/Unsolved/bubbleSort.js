...
goButton.onclick = function () {
  ...
  // for (var b = 0; b < unsortedArr.length; b++) {
  //   swapped = false;
  //   if (swapped = false) {
  //     for (var i = 0; i < unsortedArr.length; i++) {
  //       if (unsortedArr[i - 1] > (unsortedArr.length - 1)) {
  //         if (unsortedArr[i - 1] > unsortedArr[i]) {
  //           unsortedArr[i] = unsortedArr[i - 1];
  //           swapped = true;
  //         }
  //       }
  //       unsortedArr.length = (unsortedArr.length - 1);
  //     }
  //   }
  // }
  // document.getElementById("start").innerHTML = unsortedArr.join(", ");
  ...
  document.getElementById("result").innerHTML = unsortedArr.join(", ");
};


function bubbleSort(passedArr) {
  var n = passedArr.length;
  var swapped = true;

  while (swapped) {
    swapped = false;
    for (i = 0; i <= (n - 1); i++) {
      if (passedArr[i - 1] > passedArr[i]) {
        swapElem(passedArr, (i - 1), i);
        swapped = true;
      }
    }
  }
...
function swapElem(passedArr, x, y) {
  var b = passedArr[y];
  passedArr[y] = passedArr[x];
  passedArr[x] = b;
}
